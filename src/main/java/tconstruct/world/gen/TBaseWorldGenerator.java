    ...
    void generateUndergroundOres(Random random, int xChunk, int zChunk, World world) {
        ...
        if (PHConstruct.generateAluminum) {
            for (int q = 0; q <= PHConstruct.aluminumuDensity; q++) {
                xPos = xChunk + random.nextInt(16);
                yPos = PHConstruct.aluminumuMinY
                        + random.nextInt(PHConstruct.aluminumuMaxY - PHConstruct.aluminumuMinY);
                zPos = zChunk + random.nextInt(16);
            ...
            for (int z = 0; z < 16; z++) {
                for (int y = 0; y < 128; y++) {
                    ...
                    if (block != null) {
                        ...
                        if (block == Blocks.stone) world.setBlock(x + chunkX, y, z + chunkZ, Blocks.end_stone, 0, 0);
