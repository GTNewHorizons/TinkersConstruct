...
public class Crossbow extends ProjectileWeapon {
    ...
    public boolean reload(ItemStack weapon, EntityPlayer player, World world, NBTTagCompound tags) {
        ...
        // already loaded
        if (tags.getBoolean("Loaded")) return false;
    ...
    public void fire(ItemStack weapon, World world, EntityPlayer player) {
        ...
        // not loaded
        if (!tags.getBoolean("Loaded")) return;
        ...
        if (tags.hasKey("Unbreaking")) reinforced = tags.getInteger("Unbreaking");
    ...
    public IIcon getIcon(ItemStack stack, int renderPass) {
        if (!animateLayer(renderPass) && renderPass < getPartAmount()) return super.getIcon(stack, renderPass);

        if (!stack.hasTagCompound()) return super.getIcon(stack, renderPass);

        NBTTagCompound tags = stack.getTagCompound().getCompoundTag("InfiTool");
        if (tags == null || renderPass > 10) return super.getIcon(stack, renderPass);

        float progress;

        if (tags.getBoolean("Loaded")) progress = 1;
        else if (!tags.hasKey("Reloading")) return super.getIcon(stack, renderPass);
        else progress = getWindupProgress(stack, getWindupTime(stack) - tags.getInteger("Reloading"));
    ...
    public int getColorFromItemStack(ItemStack stack, int renderPass) {
        if (renderPass != 2 || !stack.hasTagCompound()) return super.getColorFromItemStack(stack, renderPass);
