...
public class WeaponryHandler {
    ...
    public void onAmmoCrafted(ToolCraftEvent.NormalTool event) {
        ...
        if (event.tool instanceof ArrowAmmo) {
            ...
            // the fletching defines the accuracy of the arrow and adds a bit breakchance. Mostly there for the
            // durability modifier because availability

            // Shortbows work better with lighter arrows
            // while Longbows require a bit heavier arrows, the lighter the arrow the more impact the accuracy has
            // otherwise
    ...
    public void onProjectileWeaponCrafted(ToolCraftEvent.NormalTool event) {
        ...
        if (event.tool instanceof BowBaseAmmo) {
            ...
            // crossbows are more efficient the higher the base drawspeed is. So the higher the drawspeed, the more
            // bonus you get
            drawSpeed *= 2.5f;
    ...
    public void buildArrow(ToolBuildEvent event) {
        if (event.headStack == null || event.handleStack == null || event.accessoryStack == null) return;
    ...
    public void buildBolt(ToolBuildEvent event) {
        if (event.headStack == null || event.handleStack == null) return;
    ...
    public void weaponryPartCrafted(PartBuilderEvent.NormalPart event) {
        ...
        if (event.pattern.getItem() == TinkerWeaponry.woodPattern) {
            ...
            if (event.pattern.getItemDamage() == 1 || event.pattern.getItemDamage() == 3) {
                ...
                if (TConstructRegistry.getBowMaterial(ms.materialID) == null) event.setResult(Event.Result.DENY);
            }
        }
        ...
        if (event.pattern.getItem() == TinkerTools.woodPattern && event.pattern.getItemDamage() == 25) {
            ...
            if (TConstructRegistry.getArrowMaterial(ms.materialID) == null) event.setResult(Event.Result.DENY);
        }
    }

    @SubscribeEvent
    public void weaponryPartCast(SmelteryCastEvent.CastingTable event) {
        if (event.recipe == null || event.recipe.output == null) return;

        if (!(event.recipe.output.getItem() instanceof IToolPart)) return;
        ...
        if (event.recipe.output.getItem() == TinkerWeaponry.arrowhead)
            if (TConstructRegistry.getArrowMaterial(mat) == null) event.setResult(Event.Result.DENY);
        // crossbow/bowlimb
        if (event.recipe.output.getItem() == TinkerWeaponry.partBowLimb
                || event.recipe.output.getItem() == TinkerWeaponry.partCrossbowLimb)
            if (TConstructRegistry.getBowMaterial(mat) == null) event.setResult(Event.Result.DENY);
    }
