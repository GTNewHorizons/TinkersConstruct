...
public abstract class HarvestTool extends ToolCore {
    ...
    public int getHarvestLevel(ItemStack stack, String toolClass) {
        ...
        if (toolClass == null || !this.getHarvestType().equals(toolClass)) return -1;
    ...
    public float getDigSpeed(ItemStack stack, Block block, int meta) {
        ...
        if (isEffective(block, meta)) return calculateStrength(tags, block, meta);
    ...
    public float calculateStrength(NBTTagCompound tags, Block block, int meta) {
        ...
        if (hlvl > tags.getInteger("HarvestLevel")) return 0.1f;
    ...
    public boolean isEffective(Block block, int meta) {
        if (this.getHarvestType().equals(block.getHarvestTool(meta))) return true;
        else return isEffective(block.getMaterial());
    }

    public boolean isEffective(Material material) {
        for (Material m : getEffectiveMaterials()) if (m == material) return true;
                ...
                if (item instanceof ItemBlock || (item != null && item == TinkerTools.openBlocksDevNull)
                    ...
                    if (item instanceof ItemBlock) {
                        ...
                        if (blockToPlace.getMaterial().blocksMovement()) {
                            if (playerBounds.intersectsWith(blockBounds)) return false;
                        }
                    }
                    ...
                    if (item == TinkerTools.openBlocksDevNull) {
                        ...
                        player.swingItem();
                    } else used = item.onItemUse(nearbyStack, player, world, x, y, z, side, clickX, clickY, clickZ);
    ...
    public void breakExtraBlock(World world, int x, int y, int z, int sidehit, EntityPlayer playerEntity, int refX,
        ...
        if (!isEffective(block, meta)) return;

        Block refBlock = world.getBlock(refX, refY, refZ);
        float refStrength = ForgeHooks.blockStrength(refBlock, player, world, refX, refY, refZ);
        float strength = ForgeHooks.blockStrength(block, player, world, x, y, z);
        if (strength == 0) strength = 0.00001F; // don't div by zero
