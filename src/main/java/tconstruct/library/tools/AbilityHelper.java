...
public class AbilityHelper {
    ...
    public static float calcKnockback(Entity user, Entity entity, ItemStack stack, ToolCore tool,
        ...
        for (ActiveToolMod mod : TConstructRegistry.activeModifiers) {
            modKnockback = mod.knockback(
                    ...
                    toolTags,
                    stack,
                    user instanceof EntityLivingBase ? (EntityLivingBase) user : null,
                    entity);
        }
    ...
    public static void damageTool(ItemStack stack, int dam, NBTTagCompound tags, EntityLivingBase entity,
        ...
        if (tags.hasKey("InfiTool") && dam > 0) // unbreaking only affects damage, not healing
        {
            ...
            if (toolTags.hasKey("Unbreaking")) {
                ...
                for (int i = dam; i > 0; i--) if (random.nextInt(10) < reinforced) dam--;
        ...
        if (ignoreCharge || !damageEnergyTool(stack, tags, entity)) {
            ...
            for (ActiveToolMod mod : TConstructRegistry.activeModifiers) {
                if (mod.damageTool(stack, dam, entity)) damagedTool = true;
            }

            if (damagedTool) return;

            int damage = tags.getCompoundTag("InfiTool").getInteger("Damage");
            // long: on a tool near max durability, damage + dam can exceed Integer.MAX_VALUE
            long damageTrue = (long) damage + dam;
    ...
    public static boolean damageEnergyTool(ItemStack stack, NBTTagCompound tags, Entity entity) {
        if (!tags.hasKey("Energy")) return false;
        ...
        if (energy != -1) {
            ...
            if (energy < usage) {
                if (energy > 0) tags.setInteger("Energy", 0);
                return false;
            }
    ...
    protected static void chargeEnergyFromHotbar(ItemStack stack, EntityPlayer player, NBTTagCompound tags) {
        ...
        for (int iter = 0; iter < 9; ++iter) {
            ...
            if (!(slot.getItem() instanceof IEnergyContainerItem)) continue;
            ...
            // if it got picked up, we're playing the sound
                ...
                if (player instanceof EntityPlayerMP) {
                    player.worldObj.playSoundAtEntity(
                            ...
                            "random.pop",
                            0.2F,
                            ((TConstruct.random.nextFloat() - TConstruct.random.nextFloat()) * 0.7F + 1.0F) * 2.0F);
    ...
    public static boolean addToInv(EntityPlayer entityplayer, ItemStack itemstack, int i, boolean flag) {
        ...
        if (flag && !flag1) {
            addItemStackToWorld(
                    ...
                    (float) Math.floor(entityplayer.posX),
                    (float) Math.floor(entityplayer.posY),
                    (float) Math.floor(entityplayer.posZ),
                    itemstack);
            return true;
        } else {
            return flag1;
    ...
    public static float calcToolSpeed(ToolCore tool, NBTTagCompound tags) {
        ...
        float speedMod = 1f;
        if (tool instanceof HarvestTool) speedMod = ((HarvestTool) tool).breakSpeedModifier();
    ...
    public static float calcStoneboundBonus(ToolCore tool, NBTTagCompound tags) {
        ...
        if (tool instanceof HarvestTool) stoneboundMod = ((HarvestTool) tool).stoneboundModifier();
