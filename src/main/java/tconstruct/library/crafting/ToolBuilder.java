...
public class ToolBuilder {
    ...
    public static void addToolRecipe(ToolCore output, Item... items) {
        if (items.length == 2) addNormalToolRecipe(output, items[0], items[1]);
        if (items.length == 3) addNormalToolRecipe(output, items[0], items[1], items[2]);
        if (items.length == 4) addNormalToolRecipe(output, items[0], items[1], items[2], items[3]);
    }
    ...
    public int getMaterialID(ItemStack stack) {
        ...
        if (item instanceof IToolPart) return ((IToolPart) item).getMaterialID(stack);
    ...
    public ItemStack buildTool(ItemStack headStack, ItemStack handleStack, ItemStack accessoryStack,
        ...
        if (head == -1) validMaterials = false;

        handle = getMaterialID(handleStack);
        if (handle == -1) validMaterials = false;

        if (!validMaterials) return null;
        ...
        if (accessory != -1) accessoryMat = TConstructRegistry.getMaterial(accessory);
        ...
        compound.getCompoundTag("InfiTool")
                .setInteger("Unbreaking", buildReinforced(headMat, handleMat, accessoryMat, extraMat));
    ...
    int buildReinforced(ToolMaterial headMat, ToolMaterial handleMat, ToolMaterial accessoryMat,
        ...
        if (dAccessory > reinforced) reinforced = dAccessory;
        if (dExtra > reinforced) reinforced = dExtra;
    ...
    public static String defaultToolName(ItemStack stack) {
        if (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("InfiTool")) return null;
        if (!(stack.getItem() instanceof ToolCore)) return null;
