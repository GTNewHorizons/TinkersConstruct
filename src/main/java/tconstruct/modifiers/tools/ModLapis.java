...
public class ModLapis extends ItemModTypeFilter {
    ...
    protected boolean canModify(ItemStack tool, ItemStack[] input) {
        if (tool.getItem() instanceof ToolCore) {
            ...
            // as above: claim an unusable input only when retrying thriftily, and only with room left
            if (amount <= 0) return !isSlotSpendingAllowed() && tags.hasKey(key) && remainingInTier(tool, max) > 0;
    ...
    public void modify(ItemStack[] input, ItemStack tool) {
        ...
        for (String s : types) {
            if (s.equals("harvest")) harvest = true;
            else if (s.equals("weapon")) weapon = true;
        }
        if (weapon) {
            if (keyPair[0] >= 450) addEnchantment(tool, Enchantment.looting, 3);
            else if (keyPair[0] >= 300) addEnchantment(tool, Enchantment.looting, 2);
            else if (keyPair[0] >= 100) addEnchantment(tool, Enchantment.looting, 1);
        }
        if (harvest) {
            if (keyPair[0] >= 450) addEnchantment(tool, Enchantment.fortune, 3);
            else if (keyPair[0] >= 300) addEnchantment(tool, Enchantment.fortune, 2);
            else if (keyPair[0] >= 100) addEnchantment(tool, Enchantment.fortune, 1);
        }
    ...
    public void midStreamModify(ItemStack tool, ToolCore toolItem) {
        ...
        if (list.contains("weapon")) {
            if (keyPair[0] >= 450) {
                ...
                if (random.nextInt(1000 - chance) == 0) addEnchantment(tool, Enchantment.looting, 3);
            } else if (keyPair[0] >= 125) {
                int chance = keyPair[0] - 175;
                if (random.nextInt(600 - chance) == 0) addEnchantment(tool, Enchantment.looting, 2);
            } else if (keyPair[0] >= 10) {
                int chance = keyPair[0] - 25;
                if (random.nextInt(250 - chance) == 0) addEnchantment(tool, Enchantment.looting, 1);
            }
        }
        ...
        if (list.contains("harvest")) {
            if (keyPair[0] >= 450) {
                ...
                int chance = keyPair[0] - 300;
                if (random.nextInt(1000 - chance) == 0) addEnchantment(tool, Enchantment.fortune, 3);
            } else if (keyPair[0] >= 125) {
                int chance = keyPair[0] - 175;
                if (random.nextInt(600 - chance) == 0) addEnchantment(tool, Enchantment.fortune, 2);
            } else if (keyPair[0] >= 10) {
                int chance = keyPair[0] - 25;
                if (random.nextInt(250 - chance) == 0) addEnchantment(tool, Enchantment.fortune, 1);
            }
        }
    }
