...
public class AModThaumicVision extends ArmorMod {
    ...
    public AModThaumicVision(ItemStack thaumometer) {
        ...
        thaumicSensesMod = new AModBoolean(
                ...
                "Thaumic Senses");
        thaumicVisionMod = new AModBoolean(
                2,
                "Thaumic Vision",
                EnumSet.of(ArmorPart.Head),
                new ItemStack[0],
    ...
    public void modify(ItemStack[] recipe, ItemStack input) {
        ...
        if (thaumicSensesMod.canModify(input, recipe)) {
            ...
            if (count > 1) thaumicSensesMod.addMatchingEffect(input);
        ...
        if (count > 0) {
            ...
            int tipNum = addToolTip(
                    input,
                    thaumicSensesMod.color + thaumicSensesMod.tooltipName,
                    thaumicSensesMod.color + thaumicSensesMod.key);
            ...
            // step 3: we simply apply thaumic vision now. it should replace the just removed tooltip and the just added
            // modifier
    ...
    private int countThaumometers(ItemStack[] recipe) {
        int count = 0;
        for (ItemStack stack : recipe) {
            if (stack != null && stack.getItem() == thaumometer) count++;
        }
        return count;
    }
}
